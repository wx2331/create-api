...
/*  79 */      } protected AppletClassLoader(URL paramURL) { super(new URL[0]);
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */     
/* 344 */     this.syncResourceAsStream = new Object();
/* 345 */     this.syncResourceAsStreamFromJar = new Object();
/*     */ 
/*     */     
/* 348 */     this.resourceAsStreamInCall = false;
/* 349 */     this.resourceAsStreamFromJarInCall = false;
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*     */ 
/*     */     
/* 773 */     this.jdk11AppletInfo = new HashMap<>();
/* 774 */     this.jdk12AppletInfo = new HashMap<>(); this.base = paramURL; this.codesource = new CodeSource(paramURL, (Certificate[])null); this.acc = AccessController.getContext(); }
...
/*     */   protected PermissionCollection getPermissions(CodeSource paramCodeSource) { Permission permission; PermissionCollection permissionCollection = super.getPermissions(paramCodeSource); URL uRL = paramCodeSource.getLocation(); String str = null; try { permission = uRL.openConnection().getPermission(); } catch (IOException iOException) { permission = null; }  if (permission instanceof FilePermission) { str = permission.getName(); } else if (permission == null && uRL.getProtocol().equals("file")) { str = uRL.getFile().replace('/', File.separatorChar); str = ParseUtil.decode(str); }  if (str != null) { String str1 = str; if (!str.endsWith(File.separator)) { int i = str.lastIndexOf(File.separatorChar); if (i != -1) { str = str.substring(0, i + 1) + "-"; permissionCollection.add(new FilePermission(str, "read")); }  }  File file = new File(str1); boolean bool = file.isDirectory(); if (this.allowRecursiveDirectoryRead && (bool || str1.toLowerCase().endsWith(".jar") || str1.toLowerCase().endsWith(".zip"))) { Permission permission1; try { permission1 = this.base.openConnection().getPermission(); } catch (IOException iOException) { permission1 = null; }  if (permission1 instanceof FilePermission) { String str2 = permission1.getName(); if (str2.endsWith(File.separator))
/*     */             str2 = str2 + "-";  permissionCollection.add(new FilePermission(str2, "read")); } else if (permission1 == null && this.base.getProtocol().equals("file")) { String str2 = this.base.getFile().replace('/', File.separatorChar); str2 = ParseUtil.decode(str2); if (str2.endsWith(File.separator))
...
/*     */         throw new IOException("open HTTP connection failed.");  }  int i = uRLConnection.getContentLength(); BufferedInputStream bufferedInputStream = new BufferedInputStream(uRLConnection.getInputStream()); try { arrayOfByte = IOUtils.readFully(bufferedInputStream, i, true); } finally { bufferedInputStream.close(); }  return arrayOfByte; }
...
/* 797 */   void setJDK12Target(Class paramClass, boolean paramBoolean) { this.jdk12AppletInfo.put(paramClass.toString(), Boolean.valueOf(paramBoolean)); }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* Location:              C:\Program Files\Java\jdk1.8.0_211\lib\tools.jar!\sun\applet\AppletClassLoader.class
